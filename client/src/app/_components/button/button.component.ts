...
const VARIANT_CLASSES: Record<ButtonVariant, string> = {
  ...
  secondary: 'bg-white text-purple-800 border border-purple-800 hover:bg-purple-50',
...
@Component({
  ...
  template: `
    <button
      [class]="baseClasses + ' ' + variantClasses() + (disabled() ? ' cursor-not-allowed opacity-50' : '')"
      [type]="type()"
      [disabled]="disabled()"
...
export class ButtonComponent {
  ...
  baseClasses = 'inline-flex items-center gap-1 px-4 py-2 rounded text-sm font-medium transition-colors cursor-pointer';
